...
fetch('https://kickout-app.fr/test_pacific/iles_pacifique.geojson')
...
.then(data => {
  ...
  // Appliquer la normalisation à chaque feature
...
const islandList = Object.entries(pacificIslands).map(([name, [lat, lon, radius, code]]) => ({
  ...
  originalLon: lon, // Conserver longitude au cas où ...
  radius,
  code
}));
...
//   L.marker([island.lat, island.lon]).addTo(map)
//     .bindPopup(`<strong>${island.name}</strong><br>Lat: ${island.lat}<br>Lon: ${island.originalLon}`);
// });

map.whenReady(() => {
  islandList.forEach(island => {
    const lat = island.lat;
    const lon = island.originalLon; // Utilise la longitude d'origine NON normalisée
...
function triggerIslandReveal() {
  ...
  function animateGrowToHold(timestamp, startTime) {
    ...
    const duration = 1000; // 1s de démarrage
  ...
  function onClickLeft() {
    if (animPhase === 'paused' && hoverIsland != null) {
      ...
      // Blanchissement de tout l'écran
      ...
      //   zoomOffset: 2,
      //   fixedPosition: true,
      //   latLng: [hoverIsland.lat,hoverIsland.lon]
      // }).addTo(map);
  ...
  function onRightClick(e) {
    ...
    // Retire fond blanc et réaffiche la loupe

...
d3.dsv(";", "DATA/toutes_thematiques.csv", row => {
  ...
  groupedByIndicator.forEach((records, indicatorCode) => {
    // Choix des transformations selon l'indicateur
    ...
    // Calcul des min/max sur les valeurs transformées si log
    ...
    // Appliquer la normalisation sur chaque enregistrement
    records.forEach(d => {
...
function updateGraph() {
  ...
  const svg = d3.select("#radarChart")
    ...
    const g = zoomGroup.append("g"); // Toutes les formes vont ici

    // Zoom behavior
    const zoom = d3.zoom()
      .scaleExtent([0.5, 3]) // Limite du zoom
      .on("zoom", (event) => {
  ...
  // Draw circular grid with values
  ...
  // Render mainIsland polygon and dots
  ...
  // Render other islands as small gray points only
  Array.from(dataByIsland.entries()).forEach(([islandCode, data]) => {
    if (islandCode === mainIslandCode) return; // Skip
  ...
  // TEXTE ADAPTATIF ANALYSE FINALE
